...
The characters are no longer generated geometry. Twelve finished models —
six men and six women from Quaternius' Ultimate Modular packs, CC0 — live in
`public/models/duelists/` with a `LICENSE.md` recording exactly where they
came from, under what terms, and which animation clips were trimmed. Do not
add a model whose license you cannot quote in that file. The old procedural
system (the lofted bodies, the painted faces) is retired from the game but
still in the tree — see the end of this file before touching it.
...
- The booth is a **model picker**: twelve duelists, three tintable garments
  each (hair among them where the model has any), one stature slider, a name.
  Every choice is a finished, photographed thing; the player combines them
  and names the result. "As made" — the model's own colour, exactly — is a
  real choice on every garment, not a swatch that happens to sit close.
...
- **Tint slots name materials.** The models carry no textures — every part is
  a named flat-colour material — so a slot is a label, a palette, and the
  exact material names it owns. `Skin`, `Eye` and `Eyebrows` are materials no
  slot may name (`UNTINTABLE`, enforced by `npm run premade`): the failure
  mode where a tint reaches a face cannot be expressed. The name lists are
  data about the vendored file, exactly like its height: swap the file,
  re-check the names.
- **Palettes are indices, never hex** — the same `CLOTH_COLORS` /
  `TRIM_COLORS` / `HAIR_COLORS` the rest of the game draws from
  (`src/story/character.ts`). A swatch can be re-tuned later without
  rewriting anybody, and a corrupt value can only ever be out of range.
...
- `public/models/duelists/*.glb` — the models themselves: mesh, rig, named
  materials, and the kept animation clips (LICENSE.md lists them — Idle, Walk
  and Run for the field, attacks, hits, a death and a wave for the duels to
  come). Self-contained; nothing fetches anything else.
- `src/components/story/premadeRig.ts` — turns a record into a duelist in a
  scene: loads and caches one template per model, recolours the materials the
  player tinted, scales to catalog height, and plays Idle/Walk/Run through an
  `AnimationMixer`. The seam it exposes is the old one: a `root` to add, one
  call per frame, a `dispose`.
- The rig rebuilds every material as cloth-rough standard ones, so the
  booth's key and the field's sun land the same way on every duelist.
...
3. **A tint must never touch the skin.** Here that rule is structural: a slot
   ...
   list, the audit's `UNTINTABLE` check has to grow with it.
4. **The authored look stays reachable, exactly.** `AS_AUTHORED` keeps the
   file's own material colour untouched. A "default" that is a nearby swatch
   is a drifting copy of a finished thing.
...
npm run premade                    # the files hold what the catalog claims —
                                   # rigged, Idle/Walk/Run aboard, every slot
                                   # naming real materials and none of them
                                   # the face's — and each file's material
                                   # inventory printed, which is what slots
...
- **Photograph every model, not the default.** The handling run taps every
  `data-pick` it finds; the frame per duelist is where a bad scale, a wrong
  facing or a lost accessory shows first.
- **Photograph the tints landing.** The handling run taps a swatch per
  garment and the as-made chip after; a slot naming the wrong material shows
  there as a colour that moved on the wrong part of the body — or nowhere.
